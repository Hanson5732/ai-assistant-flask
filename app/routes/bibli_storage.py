...
def upload_multi_papers():
    ...
    # 1. 在主线程中获取当前的 app 实例，以便传给子线程
    app = current_app._get_current_object()

    # 2. 修复：接收三个参数 (内容, 文件名, app实例)
    def process_single_file(file_content, filename, app_context):
        # 3. 修复：激活应用上下文，否则子线程里无法使用 db.session
    ...
    for file in files:
        ...
        # 4. 修复：增加传入 filename 和 app 参数
        futures.append(executor.submit(process_single_file, file_content, file.filename, app))

    results = []
    # 5. 增加异常捕获，防止任何漏网的崩溃导致 500
    try:
        ...
        import traceback
        traceback.print_exc()
        return Response.error(f"Internal Error during batch processing"), 500
